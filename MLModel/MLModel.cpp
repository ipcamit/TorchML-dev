...
void PytorchModel::SetExecutionDevice(const char * const device_name)
{
  ...
  // Default to 'cpu'
  if (device_name == nullptr) { device_name_as_str = "cpu"; }
  else
  {
...
void PytorchModel::SetInputNode(int model_input_index,
                                int * input,
                                int size,
                                bool requires_grad)
{
...
void PytorchModel::SetInputNode(int model_input_index,
                                int layer,
                                int size,
                                long ** const unrolled_graph)
{
  long * edge_index_array = unrolled_graph[layer];
  torch::Dtype torch_dtype
      = torch::kLong;  // get_torch_data_type(edge_index_array);
  torch::TensorOptions tensor_options
      = torch::TensorOptions().dtype(torch_dtype);
  torch::Tensor edge_index
      = torch::from_blob(edge_index_array, {2, size}, tensor_options)
            .to(*device_);
